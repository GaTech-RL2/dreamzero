#!/bin/bash
# DreamZero DROID Full Fine-Tuning Script with a ~300M-param image+text-to-video (i2v) DiT.
#
# Usage:
#   bash scripts/train/droid_training_300m.sh
#
# This variant keeps the SAME i2v conditioning as the Wan2.1-I2V-14B-480P backbone (CLIP first-frame
# embedding + first-frame latent concat + text) but shrinks the trainable DiT to ~300M
# (dim=896, ffn=3584, heads=7, layers=22). Because those dims no longer match any released WAN
# checkpoint, the DiT is trained FROM SCRATCH (the 300M action-head config sets
# skip_component_loading=true). The frozen T5 text encoder, CLIP image encoder, and VAE are still
# loaded from the Wan2.1-I2V-14B-480P checkpoint below.
#
# Prerequisites:
...
#   - Wan2.1-I2V-14B-480P weights (auto-downloaded or pre-downloaded) -- used ONLY for the frozen
#     T5 + CLIP + VAE; its DiT weights are not loaded.
...
# Repo root (same logic as droid_training_full_finetune_wan21.sh)
...
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_droid_300m_full_finetune"}
...
# Wan2.1-I2V-14B-480P checkpoint (provides the FROZEN CLIP image encoder, T5 text encoder, and VAE).
# The 14B DiT weights are NOT used (the 300M config trains its DiT from scratch).
...
# Full fine-tune (from-scratch DiT): train_architecture=full, save_lora_only=false, ZeRO-2 (+ optional offload).
# Higher LR than the 14B fine-tune (1e-4 vs 1e-5) since the ~300M DiT trains from scratch rather than adapts.
