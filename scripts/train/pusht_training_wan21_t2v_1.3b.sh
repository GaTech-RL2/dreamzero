#!/bin/bash
# DreamZero PushT training -- Wan2.1-T2V 1.3B DiT, FROM SCRATCH.
#
...
# From scratch: dit_version=null + skip_component_loading=true (the t2v config does NOT set it, unlike
# the 300m config), so the DiT is random-init; only the frozen T5 + VAE load from the local
# Wan2.1-I2V-14B-480P checkpoint (the umt5-xxl T5 and Wan2.1 z=16 VAE are shared across Wan2.1 models).
# Single view => frame_seqlen = (256/8/2)*(256/8/2) = 256 (overrides the default 880).
#
# Usage:
#   NUM_GPUS=4 bash scripts/train/pusht_training_wan21_t2v_1.3b.sh
#   bash scripts/train/pusht_training_wan21_t2v_1.3b.sh max_steps=2 report_to=none
...
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_pusht_1.3b"}
