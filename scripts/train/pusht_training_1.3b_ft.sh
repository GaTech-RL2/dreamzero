...
# action head. Contrast: pusht_training_wan21_t2v_1.3b.sh is FROM SCRATCH (dit_version=null, random DiT);
# pusht_training_1.3b_lora.sh freezes the DiT + trains LoRA adapters. Full-FT keeps the pretrained video
# prior but adapts every weight -> LR is LOW (1e-5) to avoid wrecking it (vs 1e-4 for LoRA/from-scratch).
#
# The frozen umt5-xxl T5 + Wan2.1 z=16 VAE are shared across Wan2.1 sizes, so they reuse the local
# Wan2.1-I2V-14B-480P copy. Single view 256x256 => frame_seqlen = (256/8/2)^2 = 256.
#
# Usage:
#   NUM_GPUS=4 PER_DEVICE_BS=16 bash scripts/train/pusht_training_1.3b_ft.sh
#   bash scripts/train/pusht_training_1.3b_ft.sh max_steps=2 report_to=none
